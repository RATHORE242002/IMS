from tkinter import*
from PIL import Image, ImageTk #pic install pillow
...
class categoryClass: 
     def __init__(self,root):
       ...
       btn_add=Button(self.root,text="ADD",command=self.add, font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=300,y=170,width=150,height=30)
     ...
     def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category  already present, try differnt",parent=self.root)
                else:
